...
#Create color palette with unique colors for each region
unique_regions = df['Region'].unique()
palette = sns.color_palette("deep", len(unique_regions))
...
def Bubble_Plot():
    ...
    scatter = plt.scatter(
        ...
        c=df['Region'].map(color_dict), #Color by region using color_dict  
        alpha=0.6,
        edgecolors='w',
        linewidth=0.5
    )
